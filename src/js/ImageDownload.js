...
function svgToCanvas(svgOriginalElement, canvasElement, callback) {
  ...
  var { svgElement, width, height } =
    cloneSvgAndFixDimensions(svgOriginalElement);
...
function downloadCanvas(canvasElement, filename) {
  ...
  if (document.createEvent) {
    ...
    console.warn(
      logPrefix +
        'Download not supported. No "document.createEvent" or "domElement.click" available.'
    );
  }
}
...
function cloneSvgAndFixDimensions(svgOriginalElement) {
  ...
  try {
    ...
    if (
      widthUnitType === SVGLength.SVG_LENGTHTYPE_UNKNOWN ||
    ...
    if (
      heightUnitType === SVGLength.SVG_LENGTHTYPE_UNKNOWN ||
    ...
    if (
      !svgElement.attributes.width ||
      regexPercentageValue.test(svgElement.attributes.width.value)
    ) {
      svgElement.setAttribute("width", boundingClientRect.width);
    }
    if (
      !svgElement.attributes.height ||
      regexPercentageValue.test(svgElement.attributes.height.value)
    ) {
