...
import Svg.Styled exposing (Svg, line, path, svg)
...
type IconOldAPI msg
    = IconOldAPI Drawing Float Color (Maybe msg) (Maybe (List Css.Style))


toSvgOldAPI : IconOldAPI msg -> Svg msg
toSvgOldAPI (IconOldAPI drawing size color maybeMsg maybeCss) =
    let
        maybeOnClick =
            case maybeMsg of
                Just msg ->
...
standardOldAPI : String -> IconOldAPI msg
standardOldAPI drawing =
    IconOldAPI drawing 24 Colors.offWhite Nothing Nothing



--withAttrs : List Attribute -> IconOldAPI msg -> IconOldAPI msg
--withAttrs attrs (IconOldAPI drawing size color maybeMsg attrs) =


withSize : Float -> IconOldAPI msg -> IconOldAPI msg
withSize newSize (IconOldAPI drawing _ color maybeMsg maybeCss) =
    IconOldAPI drawing newSize color maybeMsg maybeCss


withColor : Color -> IconOldAPI msg -> IconOldAPI msg
withColor newColor (IconOldAPI drawing size _ maybeMsg maybeCss) =
    IconOldAPI drawing size newColor maybeMsg maybeCss


withConditionalColor : Bool -> Color -> IconOldAPI msg -> IconOldAPI msg
withConditionalColor shouldApply newColor (IconOldAPI drawing size oldColor maybeMsg maybeCss) =
    if shouldApply then
        IconOldAPI drawing size newColor maybeMsg maybeCss

    else
        IconOldAPI drawing size oldColor maybeMsg maybeCss


withOnClick : msg -> IconOldAPI msg -> IconOldAPI msg
withOnClick msg (IconOldAPI drawing size color _ maybeCss) =
    IconOldAPI drawing size color (Just msg) maybeCss


withCss : List Css.Style -> IconOldAPI msg -> IconOldAPI msg
withCss attributes (IconOldAPI drawing size color maybeMsg _) =
    IconOldAPI drawing size color maybeMsg (Just attributes)
...
delete =
    ...
    standard "0 0 24 24"
        (\_ ->
            [ line (attrs ++ [ x1 "18", y1 "6", x2 "6", y2 "18" ]) []
            , line (attrs ++ [ x1 "6", y1 "6", x2 "18", y2 "18" ]) []
            ]
        )


standard : String -> (Color -> List (Svg msg)) -> Icon msg
standard viewBox structure =
    { structure = structure
...
toSvg { structure, color, viewBox, size, onClick } =
    let
        maybeOnClick =
            onClick
                |> Maybe.map (\msg -> [ stopPropagationOn "click" (Decode.map alwaysStopPropagation (Decode.succeed msg)) ])
                |> Maybe.withDefault []
    ...
    svg
        ([ Attrs.viewBox viewBox
         ...
         , Attrs.color (Colors.toString color)
         , Attrs.stroke (Colors.toString color)
         ]
            ++ maybeOnClick
        )
        (structure color)
